...
Since we rescaled $\phi$ by $\frac{1}{\sqrt{2m}}$ to get $\varphi$, the in/out states are also changed. We must multiply them by $\sqrt{2E}$ to compensate that change. 
...
\subsubsection{NRSQED}
 \begin{align*}
   i\mathcal{M}_{NRSQED}^{(0)}&=\feynmandiagram[horizontal=i1 to f1,layered layout,inline=($(a)!0.5!(c)$),medium]{
	...
	i2[particle=$p_1$] -- [] c -- [] f2[particle=$p_2$],
	{ [same layer] a -- [photon,momentum'=$q$] c},
  };=-2\sqrt{E_1E_2}e^2v^{0}\frac{i(1+\frac{E_1+E_2}{2m})}{\vb{q}^2}
\end{align*}
\subsection{NLO}
\subsubsection{SQED}
\begin{align*}
