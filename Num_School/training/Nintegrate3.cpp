/*
CC -o Nintegrate Nintegrate3.cpp 
*/
...
{
        stat_obj tmp;
        //calculate the error based on the jackknifed samples;
        int size=res.size();
        double aver=0.0;
        for(int i=0;i<size;i++)
            aver+=res[i]/size;
...
//Vegas_simple;
            ...
            //
            static normal_distribution<> gau(0,sigma);
            static mt19937 mt;
            //
        	...
        	for(int i=0;i<calls;i++)
        	{
                ...
                double P=g(x,dt);
                for(int id=0;id<dimen;id++)
                    P*=(sqrt(2*M_PI)*sigma)*exp(pow(x[id]/sigma,2)/2);
