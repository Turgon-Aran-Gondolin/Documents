...
\end{align}
There're following 8 diagrams. The corresponding Feynman rules are: 
\begin{align}
...
\end{align}
The last line stands for the momentum conservation between two dots. There're also an extra $1/2$ factor on the outside and a Dirac delta function to eliminate all $z$-direction loop momenta. 
...
\begin{align}
	\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		...
		\end{feynman}
	\end{tikzpicture}&=\frac{1}{2}\bar u(P) \pqty{-ig_st^a\gamma_\nu} \int\mm{l}\frac{i}{\slashed l+i\epsilon}\gamma^z\frac{-ig^{\mu \nu}}{(P-l)^2} \frac{i}{\slashed l+i\epsilon}\pqty{-ig_s\gamma_{\mu}t^a}u(P)\delta(l^z-(1-x)P^z)\notag\\
	&=-i\frac{g_s^2C_F}{2}\int\mm{l}\bar u(P) \gamma^\mu \frac{\slashed l-m}{l^2-m^2+i\epsilon}\gamma^z\frac{\slashed l-m}{l^2-m^2+i\epsilon}\gamma_{\mu} u(P)\frac{1}{(P-l)^2}\delta(l^z-(1-x)P^z)
\end{align}
After spin sum: 
\begin{align}
	\sum_s\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		\begin{feynman}
			\node[dot] (a);
			\node[right=\FDWidthS of a,dot] (b);
			\vertex[below=\FDHeightS of a] (exa);
		...
		\end{feynman}
	\end{tikzpicture}&=-i\frac{g_s^2C_F}{2}\int\mm{l}\sum_s\bar u(P) \gamma^\mu \frac{\slashed l-m}{l^2-m^2+i\epsilon}\gamma^z\frac{\slashed l-m}{l^2-m^2+i\epsilon}\gamma_{\mu} u(P)\frac{1}{(P-l)^2}\delta(l^z-(1-x)P^z)
\end{align}
Diagram~\ref{2/-} gives
\begin{align}
	\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		...
		\end{feynman}
	\end{tikzpicture}&=\frac{1}{2}\bar u(P) \gamma^z \int\mm{l}\frac{i}{l^z-P^z+i\epsilon}\pqty{-ig_st^a}\frac{-ig^{\mu z}}{(P-l)^2} \frac{i}{\slashed l+i\epsilon}\pqty{-ig_s\gamma_{\mu}t^a}u(P)\delta(l^z-(1-x)P^z)\notag\\
	&=\frac{g_s^2C_F}{2}
\end{align}
...
\begin{align*}
	...
	=&\bar u(P)e^{iP\cdot z}\int\mm{l}\tilde D_F(l)\tilde D_G^{\mu z}(P-l)\frac{i}{P^z-l^z+i\epsilon}u(P)
\end{align*}
The complete quasi PDF at one loop is
\begin{align}
	&-\frac{g_s^2C_F\gamma_{\mu}}{2} \int \frac{\dd z}{2 \pi} e^{i x P^{z} z}\bar u(P)e^{iP\cdot z}\int\mm{l}\tilde D_F(l)\tilde D_G^{\mu z}(P-l)\frac{i}{P^z-l^z+i\epsilon}u(P)\notag\\
	=&-\frac{g_s^2C_F\gamma_{\mu}}{2 P^z}\bar u(P)\int\mm{l}\tilde D_F(l)\tilde D_G^{\mu z}(P-l)\frac{i}{P^z-l^z+i\epsilon}u(P)\delta(1-x)\notag
\end{align}
...
\begin{align*}
	...
	\int\frac{\dd w}{2\pi}\frac{ie^{-iwz' }}{w+i\epsilon}\int\frac{\dd h}{2\pi}\frac{ie^{-ih(z''-z') }}{h+i\epsilon}\\
	=&\bar u(P)\int \frac{\dd z}{2 \pi} e^{-i (1-x) P^{z} z+il^zz} \gamma^z \int \dd z^{\prime} \int \dd z^{\prime\prime} \int\mm{l}\tilde D_G^{zz}(l) 
	\int\frac{\dd w}{2\pi}\frac{i}{w+i\epsilon}\int\frac{\dd h}{2\pi}\frac{i}{h+i\epsilon}e^{-i(w-l-h)\cdot z'}e^{-i(h+l)\cdot z''} u(P)\\
	=&\bar u(P) \gamma^z \int\mm{l}\tilde D_G^{zz}(l) 
...
\begin{align}
	\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		...
		\end{feynman}
	\end{tikzpicture}=-\frac{g_s^2C_F}{4}\bar u(P) \gamma^z \int\mm{l}\tilde D_G^{zz}(l) 
	\frac{i}{0+i\epsilon}\frac{i}{-l^z+i\epsilon} \delta(l^z-(1-x)P^z)u(P)
\end{align}
Diagram~\ref{2o-} is 
\begin{align}
...
\end{align}
and it behaves exactly like \ref{2-o}, since those extra $(+z)$s will be cancelled in the Wick contraction. 
