...
\begin{align}
	\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		...
		\end{feynman}
	\end{tikzpicture}=\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		\begin{feynman}
			\node[dot] (a);
			\node[right=\FDWidthS of a,dot] (b);
			\vertex[below=\FDHeightS of a] (exa);
...
\end{align}}. By redefining the plus function to an extended version: 
\begin{align}
	f_+(x)=f(x)-\delta(1-x)\int_{-\infty}^\infty\dd yf(y)
\end{align}
The full result is 
\begin{align}
...
\begin{align}
	\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		...
		\end{feynman}
	\end{tikzpicture}&=\frac{1}{2P^z}\mm{l}\bar u(P)(-ig_s\gamma_{\mu}t^a)\frac{i(\slashed l+m)}{l^2-m^2+i\epsilon}\gamma^z\frac{-ig^{\mu z}}{(P-l)^2+i\epsilon}\frac{i}{P^z-l^z}(ig_st^a)u(P)\delta(1-x)\notag\\
	&=\frac{-ig_s^2C_F}{2P^z}\int\mm{l}\bar u(P)\gamma^z\frac{\slashed l+m}{l^2-m^2+i\epsilon}\gamma^zu(P)\frac{1}{l^z-P^z-i\epsilon}\frac{1}{(P-l)^2+i\epsilon}\delta(1-x)\notag\\
	&=\begin{tikzpicture}[baseline=($(a)!0.5!(exa)$.base)]
		\begin{feynman}
			\node[dot] (a);
...
\end{align}
where $Z_F$ is independent of $x$ and $y$. In axial gauge, $Z_F$ is the fermion self energy, while in Feynman gauge, it's the combination of fermion self energy, gauge link self energy and loops between quark line and gauge link line involving only one side of the uncut diagram. 
...
\end{align}
Here we use a modified version of plus function
\begin{align}
	f_+(x)&=f(x)-\delta(1-x)\int_{-\infty}^\infty\dd yf(y)\\
